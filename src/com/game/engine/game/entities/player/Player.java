...
import com.game.engine.game.particles.FootstepParticle;
import com.game.engine.game.particles.PlayerParticle;
import com.game.engine.game.particles.SwordParticle;
...
public class Player extends GameObject {
    ...
    public void update(EngineAPI api, GameState gs, float dt) {
        ...
        if(health == 0) {
            ((InventoryComponent)(this.findComponent("inventory"))).deathDrop(gs);
            this.setDead(true);
        }
        ...
        if(api.getInput().isButtonDown(MouseEvent.BUTTON1)) {
            if(((InventoryComponent)this.findComponent("inventory")).getHeldItem().getNamespacedID().equals("game-exe:bamboo_sword")) {
