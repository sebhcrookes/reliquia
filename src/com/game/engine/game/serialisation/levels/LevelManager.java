...
public class LevelManager {
    ...
    public void loadLevel(int levelNumber) {
        ...
        if(sessionLoads != 0 || gs.getObjectManager().getObject("game-exe:player").getTileX() == -1) { // If we are loading a new level
            gs.getObjectManager().getObject("game-exe:player").setTileX(LevelManager.spawnX);
            gs.getObjectManager().getObject("game-exe:player").setTileY(LevelManager.spawnY);
            gs.getObjectManager().getObject("game-exe:player").setOffX(0);
            gs.getObjectManager().getObject("game-exe:player").setOffY(0);
            gs.getObjectManager().getObject("game-exe:player").setFacing(0);
        }
