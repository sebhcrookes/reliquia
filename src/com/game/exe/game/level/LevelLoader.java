...
public class LevelLoader {
    ...
    public void load(int levelNumber) {
        ...
        try {
            ...
            this.levelW = levelImage.getW();
            this.levelH = levelImage.getH();

            lm.setLevelW(this.levelW);
            lm.setLevelH(this.levelH);
            ...
            for (int y = 0; y < levelImage.getH(); y++) {
                for (int x = 0; x < levelImage.getW(); x++) {
                    ...
                    if(gm.getCollisionValue(currentBlock) == "bamboostem") {
                        if(gm.getCollisionValue(currentBlock - levelImage.getW()) == "air") {
                    ...
                    //region Grass Block
                    if(gm.getCollisionFromID(gm.getCollisionValue(currentBlock)) && gm.getCollisionValue(currentBlock - 1) == "grassblockright") {
                    ...
                    if(gm.getCollisionValue(currentBlock) == "grassblock") {
                        for(int n = 0; n < gm.blocks.blockList.size(); n++) {
                            if(gm.blocks.blockList.get(n).blockID == gm.getCollisionValue(currentBlock-1)) {
                    ...
                    if(gm.getCollisionValue(currentBlock) == "dirtblock") {
                        if(gm.getCollisionValue(currentBlock-1) == "grassblockright") {
                            gm.setCollisionValue("grassblock", currentBlock-1);
                        }
                    }
                    //endregion
                    ...
                    //Tree Generation
                    if(gm.getBlockNumberFromID(gm.getCollisionValue(currentBlock)) == 1 && !gm.getCollisionFromID(gm.getCollisionValue(currentBlock - this.levelW))) {
                        if(gm.getBlockNumberFromID(gm.getCollisionValue(currentBlock-1)) == 1 && !gm.getCollisionFromID(gm.getCollisionValue(currentBlock - (this.levelW - 1)))) {
                            if(gm.getBlockNumberFromID(gm.getCollisionValue(currentBlock-2)) == 1 && !gm.getCollisionFromID(gm.getCollisionValue(currentBlock - (this.levelW - 2)))) {
                                if(gm.getBlockNumberFromID(gm.getCollisionValue(currentBlock-3)) == 1 && !gm.getCollisionFromID(gm.getCollisionValue(currentBlock - (this.levelW - 3)))) {
                                    if(gm.getBlockNumberFromID(gm.getCollisionValue(currentBlock-4)) == 1 && !gm.getCollisionFromID(gm.getCollisionValue(currentBlock - (this.levelW - 4)))) {
                                        if(gm.getBlockNumberFromID(gm.getCollisionValue(currentBlock-5)) == 1 && !gm.getCollisionFromID(gm.getCollisionValue(currentBlock - (this.levelW - 5)))) {
                                            if(gm.getBlockNumberFromID(gm.getCollisionValue(currentBlock-6)) == 1 && !gm.getCollisionFromID(gm.getCollisionValue(currentBlock - (this.levelW - 6)))) {
                                                if(treeCount < 2) {
